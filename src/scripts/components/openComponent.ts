...
export type OpenSaveObject = PathSaveObject & {
	poles?: Pole
	scale?: number
}

export class OpenComponent extends Voltageable(PathLabelable(PathComponent)) {
	private static jsonID = "open"
	static {
		CircuitComponent.jsonSaveMap.set(OpenComponent.jsonID, OpenComponent)
	}
	...
	constructor() {
		...
		this.componentVisualization = CanvasController.instance.canvas.line().fill("none").stroke("none")
	...
	public toJson(): OpenSaveObject {
		let data = super.toJson() as OpenSaveObject
		data.type = OpenComponent.jsonID
	...
	public copyForPlacement(): OpenComponent {
	...
	public applyJson(saveObject: OpenSaveObject): void {
	...
	public static fromJson(saveObject: OpenSaveObject): OpenComponent {
		return new OpenComponent()
	}
